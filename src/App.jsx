...
export default function App() {
  ...
  return (
    <main className="min-h-dvh bg-[radial-gradient(circle_at_top,#d9f99d,transparent_28%),linear-gradient(180deg,#f8fafc_0%,#eef2ff_45%,#e2e8f0_100%)] px-4 py-6 text-slate-900">
      <div className="mx-auto w-full max-w-[460px] space-y-4">
        ...
        <section className="rounded-[28px] border border-white/70 bg-white/85 p-5 shadow-[0_18px_60px_-36px_rgba(15,23,42,0.55)] backdrop-blur">
          <p className="text-[11px] uppercase tracking-[0.22em] text-slate-400">{currentView.eyebrow}</p>
          <h2 className="mt-2 text-2xl font-black text-slate-900">{currentView.title}</h2>
          <p className="mt-2 text-sm leading-6 text-slate-600">{currentView.description}</p>

          <div className="mt-5 rounded-3xl border border-dashed border-slate-300 bg-slate-50/80 p-6 text-sm text-slate-500">
            {activeView === "home"
